import {
  uniqueProductIdentifierCreateRequestPlainFactory,
  uniqueProductIdentifierUpdateRequestPlainFactory,
...
describe("UniqueProductIdentifiersNamespace", () => {
  it("list() calls GET /unique-product-identifiers", async () => {
    ...
    await ns.list();
    expect(axios.get).toHaveBeenCalledWith("/unique-product-identifiers", { params: undefined });
  });
  ...
  it("create() calls POST /unique-product-identifiers with verbatim body", async () => {
    const { ns, axios } = makeNamespace();
    const body = uniqueProductIdentifierCreateRequestPlainFactory.build(
      {},
      { transient: { batch: "LOT-1", serial: "SN-1" } },
    );
    await ns.create(body);
    expect(axios.post).toHaveBeenCalledWith("/unique-product-identifiers", body);
  });

  it("createInternal() calls POST /unique-product-identifiers/internal with verbatim body", async () => {
    const { ns, axios } = makeNamespace();
    const body = { referenceId: "p-1" };
    await ns.createInternal(body);
    expect(axios.post).toHaveBeenCalledWith("/unique-product-identifiers/internal", body);
  });

  it("update('upi-1', {gtin, batch:'LOT-2'}) calls PATCH /unique-product-identifiers/upi-1 with verbatim body", async () => {
    const { ns, axios } = makeNamespace();
    const body = uniqueProductIdentifierUpdateRequestPlainFactory.build(
      {},
      { transient: { batch: "LOT-2" } },
    );
    await ns.update("upi-1", body);
    expect(axios.patch).toHaveBeenCalledWith("/unique-product-identifiers/upi-1", body);
  });
  ...
  it("delete('upi-1') calls DELETE /unique-product-identifiers/upi-1", async () => {
    ...
    expect(axios.delete).toHaveBeenCalledWith("/unique-product-identifiers/upi-1");
  });

  it("delete('a/b') encodes the segment to /unique-product-identifiers/a%2Fb", async () => {
    const { ns, axios } = makeNamespace();
    await ns.delete("a/b");
    expect(axios.delete).toHaveBeenCalledWith("/unique-product-identifiers/a%2Fb");
  });
});
