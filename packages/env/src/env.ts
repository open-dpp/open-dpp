...
export const envSchema = z
  .object({
    ...
    OPEN_DPP_S3_PROFILE_PICTURE_BUCKET: z.coerce
      .string()
      .optional()
    ...
    OPEN_DPP_JSON_LIMIT_INTEGRATION: z.coerce.string().or(z.number()).optional().default("50mb"),
    // Mail
  ...
  .superRefine((val, ctx) => {
    ...
    if (isAuthCloudEnabled) {
      ...
      if (
        !hasAuthCloudProvider ||
        !hasAuthCloudClientId ||
        !hasAuthCloudClientSecret ||
        !hasAuthCloudDiscoveryUrl
      ) {
        ctx.addIssue({
          code: "custom",
          message:
            "OPEN_DPP_AUTH_CLOUD_ENABLED is set to true but not correctly configured. Please check the following env variables:",
          path: [
    ...
    if (
      (hasAuthAdminUsername && !hasAuthAdminPassword) ||
      (!hasAuthAdminUsername && hasAuthAdminPassword)
    ) {
      ctx.addIssue({
        code: "custom",
        message:
          "Provide both OPEN_DPP_AUTH_ADMIN_USERNAME and OPEN_DPP_AUTH_ADMIN_PASSWORD if you provide at least one of both.",
...
export function validateEnv(env: Record<string, any>): Record<string, any> {
  ...
  if (result.error) {
    throw new Error(`Environment is not properly configured: \n${z.prettifyError(result.error)}`);
  }
