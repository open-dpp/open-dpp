...
const gs1ResolverBase = "https://id.acme.com";
...
describe("PermalinkDtoSchema polymorphism", () => {
  ...
  it("parses a presentation permalink with all new fields", () => {
    ...
    if (result.success) {
      ...
      expect(result.data.gs1ResolverBase).toBeNull();
  ...
  it("parses a gs1-link permalink", () => {
    const result = PermalinkDtoSchema.safeParse({
      ...
      primary: false,
      gs1ResolverBase: gs1ResolverBase,
    ...
    if (result.success) {
      ...
      expect(result.data.gs1ResolverBase).toBe(gs1ResolverBase);
  ...
  // (6b) rejects a presentation permalink with non-null gs1ResolverBase
  it("rejects a presentation permalink with non-null gs1ResolverBase", () => {
    const result = PermalinkDtoSchema.safeParse({
      ...base,
      kind: "presentation",
      presentationConfigurationId: configId,
  ...
  it("parses a legacy doc lacking kind/primary/uniqueProductIdentifierId/gs1* with defaults", () => {
    const result = PermalinkDtoSchema.safeParse({
      ...base,
      presentationConfigurationId: configId,
      // no kind, no primary, no uniqueProductIdentifierId, no gs1ResolverBase, no gs1DataAttributes
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.kind).toBe("presentation");
...
describe("PermalinkCreateRequestSchema", () => {
  ...
  // (2) parses gs1-link {kind:'gs1-link', uniqueProductIdentifierId:<uuid>, presentationConfigurationId?:<uuid>|null, gs1ResolverBase?, gs1DataAttributes?, slug?}
  ...
  it("parses a gs1-link create request with optional presentationConfigurationId, gs1ResolverBase, gs1DataAttributes, and slug", () => {
    const result = PermalinkCreateRequestSchema.safeParse({
      kind: "gs1-link",
      uniqueProductIdentifierId: upiId,
      presentationConfigurationId: configId,
  ...
  // (4) rejects presentation with gs1ResolverBase/gs1DataAttributes
  it("rejects a presentation create request with gs1ResolverBase", () => {
    const result = PermalinkCreateRequestSchema.safeParse({
      kind: "presentation",
      presentationConfigurationId: configId,
      gs1ResolverBase: "https://id.acme.com",
...
describe("PermalinkUpdateRequestSchema", () => {
  ...
  // (7) accepts {gs1ResolverBase, gs1DataAttributes}
  it("accepts gs1ResolverBase and gs1DataAttributes", () => {
    const result = PermalinkUpdateRequestSchema.safeParse({
      gs1ResolverBase: "https://id.acme.com",
      gs1DataAttributes: { "17": "251231" },
    });
