...
- `POST /organizations` accepts `{ name, owner? }` and validates the name (non-empty after trimming); the optional `owner` is restricted to instance admins (see the owner-provisioning entry). A `slug` in the body is ignored; `logo` and `metadata` are no longer part of the create type (they were never forwarded).
