      ...
      <div v-if="productPassport" class="border-t border-gray-100">
        <dl class="divide-y divide-gray-100">
          <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            ...
            <template v-if="productPassportStore.organizationImage">
              <dt class="text-sm font-medium text-gray-900">
                {{ t("organizations.form.image.shortLabel") }}
              </dt>
              <dd class="mt-1 text-sm/6 text-gray-700 sm:col-span-2 sm:mt-0">
                <Image
                  :src="productPassportStore.organizationImage.url"
                  :alt="productPassport.organizationName || t('organizations.form.image.shortLabel')"
                  width="100%"
                  image-class="max-h-[250px]"
                  preview
                />
              </dd>
            </template>
