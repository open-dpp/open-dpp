...
export const ColumnCreateEditorPropsSchema = z.discriminatedUnion("modelType", [
  ...
  z.object({ modelType: z.literal(AasSubmodelElements.ReferenceElement) }),
]);
...
export function useAasDrawer({ onHideDrawer, can, isArchived }: AasDrawerProps): IAasDrawer {
  ...
  const openDrawer: OpenDrawerCallback<EditorType, EditorModeType> = ({
    ...
    if (toValue(isArchived)) {
      ...
      saveButtonIsVisible.value = activePath.value.idShortPathIncludingSubmodel
        ? can(Permissions.Edit, activePath.value.idShortPathIncludingSubmodel)
        : true;
    }
  };
