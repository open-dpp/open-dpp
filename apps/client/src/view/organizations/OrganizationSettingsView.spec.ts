    ...
    en: {
      ...
      organizations: {
        ...
        form: {
          ...
          gs1ResolverBaseUrl: {
            label: "GS1 resolver domain",
            description: "Domain used to build GS1 Digital Links for this organization.",
          },
          updateError: "Failed",
          updateSuccess: "Saved",
        },
...
describe("OrganizationSettingsView — GS1 resolver override", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    getBranding.mockResolvedValue({
      data: { logo: null, primaryColor: null, permalinkBaseUrl: null, gs1ResolverBaseUrl: null },
    });
  ...
  it("renders the GS1 resolver domain override field", async () => {
    const wrapper = mountView();
    await flushPromises();
    expect(wrapper.find("#gs1ResolverBaseUrl").exists()).toBe(true);
  });

  it("sends the gs1ResolverBaseUrl on save", async () => {
    getBranding.mockResolvedValue({
      data: {
        logo: null,
        primaryColor: null,
  ...
  it("normalises a blank gs1ResolverBaseUrl to null on save", async () => {
    getBranding.mockResolvedValue({
      data: {
        logo: null,
        primaryColor: null,
    ...
    expect(setBranding).toHaveBeenCalledTimes(1);
    expect(setBranding.mock.calls[0]![0].gs1ResolverBaseUrl).toBeNull();
  });
});
