...
const { listPermalinks, deletePermalink, setPrimaryPermalink, confirmRequire, routerPush, routerReplace } =
  vi.hoisted(() => ({
...
describe("PermalinkListView delete + set-primary (Slice 76)", () => {
  beforeEach(() => {
    ...
    confirmRequire.mockImplementation(
      (opts: { accept?: () => void | Promise<void> }) => opts.accept?.(),
    );
  });
  ...
  describe("(a) Set-primary", () => {
    it("non-primary presentation row shows 'Set primary' button", async () => {
      listPermalinks.mockResolvedValueOnce({
        data: { paging_metadata: { cursor: null }, result: [PRIMARY_PRESENTATION, NON_PRIMARY_PRESENTATION] },
      });
      ...
      const btn = wrapper.find(`[data-testid="permalink-set-primary-btn-${NON_PRIMARY_PRESENTATION.id}"]`);
    ...
    it("primary row has no 'Set primary' button", async () => {
      ...
      const btn = wrapper.find(`[data-testid="permalink-set-primary-btn-${PRIMARY_PRESENTATION.id}"]`);
  ...
  describe("(b) Delete BLOCKED", () => {
    it("published permalink: Delete button is absent or disabled", async () => {
      listPermalinks.mockResolvedValueOnce({
        data: { paging_metadata: { cursor: null }, result: [PRIMARY_PRESENTATION, PUBLISHED_PRESENTATION] },
      });
  ...
  describe("(c) Delete ALLOWED", () => {
    ...
    it("non-primary unpublished presentation row: Delete button is present and enabled", async () => {
      ...
      const btn = wrapper.find(`[data-testid="permalink-delete-btn-${NON_PRIMARY_PRESENTATION.id}"]`);
      expect(btn.exists()).toBe(true);
      expect(btn.attributes("disabled")).toBeUndefined();
    });
