...
const CreateDialogStub = defineComponent({
  ...
  props: ["visible", "draftPassports", "createGs1Upi"],
...
const GS1_UPI: UniqueProductIdentifierListItemDto = {
  uuid: "gs1-upi-uuid",
  referenceId: "ref-222",
  type: "GS1",
  gtin: "04006381333931",
...
describe("UniqueProductIdentifierListView", () => {
  ...
  it("(2a) system row is READ-ONLY: has no delete action (data-testid upi-row-system has no delete)", async () => {
    listUpis.mockResolvedValueOnce({
    ...
    const systemRow = wrapper.find("[data-testid='upi-row-system']");
    expect(systemRow.exists()).toBe(true);
    expect(systemRow.find("[data-testid='upi-delete-btn']").exists()).toBe(false);
  });
  ...
  it("(4a) after create dialog emits 'created', Gs1DigitalLinkPromptDialog is visible with the new UPI", async () => {
    ...
    expect(promptDialog.find("[data-testid='gs1-link-prompt-upi-uuid']").text()).toBe("new-upi-uuid");
  });
