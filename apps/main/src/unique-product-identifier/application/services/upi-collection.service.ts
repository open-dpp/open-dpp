...
export class UpiCollectionService {
   ...
   *
   * System (OPEN_DPP_UUID) rows are read-only and always rejected with 409.
   *
   * @throws NotFoundException when the UPI does not exist.
   * @throws ConflictException when the UPI is a system row (read-only).
   * @throws ConflictException when the passport is published (lifecycle freeze).
  ...
  async update(
    ...
    // System rows are read-only — checked before the draft gate.
    if (upi.type !== ExternalIdentifierType.GS1) {
      throw new ConflictException("System unique product identifiers are read-only");
    }
  ...
  /**
   * Delete a single GS1 UPI, but only while the referenced passport is a draft.
   *
   * System (OPEN_DPP_UUID) rows are read-only and always rejected with 409.
   * Uses a single-id delete that does NOT touch sibling UPIs.
   *
   * @throws NotFoundException when the UPI does not exist.
   * @throws ConflictException when the UPI is a system row (read-only).
   * @throws ConflictException when the passport is published (lifecycle freeze).
  ...
  async delete(uuid: string): Promise<void> {
    ...
    if (!passport.isDraft()) {
      throw new ConflictException(
        "A GS1 UPI can only be deleted while the passport is a draft; it is locked once the passport is published",
      );
    }
