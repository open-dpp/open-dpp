...
export class OrganizationsController {
    ...
    async create(
        ...
        const user = await this.userService.findOne(req.authContext.keycloakUser.sub);
        if (!user) {
    ...
    @Get(':id')
    async findOne(@Param('id') id: string, @Request() req: authRequest.AuthRequest) {
    ...
    @Get(':id/members')
    async getMembers(@Param('id') id: string, @Request() req: authRequest.AuthRequest) {
    await this.permissionsService.canAccessOrganizationOrFail(
      id,
      req.authContext,
    );
