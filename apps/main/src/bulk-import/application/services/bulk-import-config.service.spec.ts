...
describe("BulkImportConfigService", () => {
  function buildFakes() {
    const configRepository = {
      save: jest.fn<(config: BulkImportConfig, options?: DbSessionOptions) => Promise<BulkImportConfig>>(),
      ...
      deleteAllByTemplateId: jest.fn<(templateId: string, options?: DbSessionOptions) => Promise<void>>(),
    };
    const runRepository = {
  ...
  it("createConfig saves a config scoped to the owning organization's template", async () => {
    ...
    templateRepository.findOneOrFail.mockResolvedValue({ getOrganizationId: () => "my-org" } as Template);
  ...
  it("updateConfig rejects a config from another organization", async () => {
    ...
    await expect(
      service.updateConfig(config.id, "my-org", { name: "renamed" }),
    ).rejects.toThrow(ForbiddenException);
  });

  it("updateConfig mutates and re-saves the owned config", async () => {
    const { service, configRepository } = buildFakes();
    const config = BulkImportConfig.create({
      organizationId: "my-org",
      templateId: randomUUID(),
  ...
  it("deleteConfig cascades to the config's runs and their items atomically before deleting it", async () => {
  ...
  it("deleteAllByTemplateId cascades across every config targeting the template", async () => {
    ...
    runRepository.deleteAllByBulkImportConfigId.mockImplementation(async (id: string) => [`run-of-${id}`]);
    ...
    expect(runItemRepository.deleteAllByRunIds).toHaveBeenCalledWith([`run-of-${c1.id}`], undefined);
    expect(runItemRepository.deleteAllByRunIds).toHaveBeenCalledWith([`run-of-${c2.id}`], undefined);
