...
describe("PermalinkRepository", () => {
  ...
  // Slice 20: GS1-link fields (uniqueProductIdentifierId, gs1ResolverBase, gs1DataAttributes)
  ...
  it("persists and round-trips a GS1 Digital Link permalink", async () => {
    ...
    const permalink = Permalink.create({
      ...
      gs1DataAttributes: { "17": "251231", "3103": "000189" },
    ...
    expect(found.gs1ResolverBase).toBe("https://id.example.com");
    expect(found.gs1DataAttributes).toEqual({ "17": "251231", "3103": "000189" });
    expect(found.presentationConfigurationId).toBeNull();
  });
  ...
  describe("findPrimaryByPassportId", () => {
    ...
    it("returns undefined when no primary permalink exists for the passport", async () => {
      ...
      await presentationConfigurationRepository.save(config);
      await repository.save(
        Permalink.create({ presentationConfigurationId: config.id, organizationId, primary: false }),
      );
  ...
  describe("findAllByOrganizationId", () => {
    it("returns all permalinks for the given org, excluding other orgs, sorted createdAt desc", async () => {
      ...
      const pA1 = Permalink.create({ presentationConfigurationId: randomUUID(), organizationId: orgA });
      const pA2 = Permalink.create({ presentationConfigurationId: randomUUID(), organizationId: orgA });
      ...
      const pB = Permalink.create({ presentationConfigurationId: randomUUID(), organizationId: orgB });
    ...
    it("paginates with cursor and returns remainder on second page (no overlap)", async () => {
      ...
      const p1 = Permalink.create({ presentationConfigurationId: randomUUID(), organizationId: orgA });
      const p2 = Permalink.create({ presentationConfigurationId: randomUUID(), organizationId: orgA });
      const p3 = Permalink.create({ presentationConfigurationId: randomUUID(), organizationId: orgA });
  ...
  describe("Slice 25 — schema version 1.3.0 + migrate-on-read", () => {
    it("saves a permalink at the latest schema version (1.3.0) with primary defaulted false and GS1 fields null/absent", async () => {
      ...
      const rawDoc = await connection.collection("permalinks").findOne({ _id: permalink.id as any });
      ...
      expect(rawDoc?.gs1ResolverBase ?? null).toBeNull();
    ...
    it("migrates a legacy permalink (1.2.0, no primary/GS1 fields) to primary=false with null GS1 fields on findOneOrFail", async () => {
      ...
      expect(found.gs1ResolverBase).toBeNull();
    ...
    it("migration is idempotent on read (raw stored _schemaVersion is still 1.2.0 after two reads)", async () => {
      ...
      expect(first.gs1ResolverBase).toBe(second.gs1ResolverBase);
