...
describe("dppStatus", () => {
  it("should be published", () => {
    const statusChange = publishDpp(DppStatusChange.create({
    ...
    expect(() => publishDpp(DppStatusChange.create({
  ...
  it("should be archived", () => {
    let statusChange = archiveDpp(DppStatusChange.create({
      currentStatus: DppStatus.Draft,
    }));
    expect(statusChange.previousStatus).toEqual(DppStatus.Draft);
    expect(statusChange.currentStatus).toEqual(DppStatus.Archived);

    statusChange = archiveDpp(DppStatusChange.create({
      currentStatus: DppStatus.Published,
    }));
    expect(statusChange.previousStatus).toEqual(DppStatus.Published);
    expect(statusChange.currentStatus).toEqual(DppStatus.Archived);

    expect(() => archiveDpp(DppStatusChange.create({
      currentStatus: DppStatus.Archived,
    }))).toThrow(
      "A dpp can only archived once.",
    );
  });

  it("should be restored", () => {
    let statusChange = restoreDpp(DppStatusChange.create({
    ...
    statusChange = restoreDpp(DppStatusChange.create({
    ...
    expect(() => restoreDpp(DppStatusChange.create({
      currentStatus: DppStatus.Archived,
    }))).toThrow(
      "No previous status to restore from.",
    );
    ...
    expect(() => restoreDpp(DppStatusChange.create({
      ...
      "Cannot restore to the archived status.",
    );
  });
});
