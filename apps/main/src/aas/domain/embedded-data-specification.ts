...
export class EmbeddedDataSpecification implements IVisitable {
  private constructor(public readonly dataSpecification: Reference) {}

  static create(data: { dataSpecification: Reference }) {
    return new EmbeddedDataSpecification(data.dataSpecification);
  }

  static fromPlain(json: unknown): EmbeddedDataSpecification {
    const parsed = EmbeddedDataSpecificationJsonSchema.parse(json);
    return new EmbeddedDataSpecification(Reference.fromPlain(parsed.dataSpecification));
  }
