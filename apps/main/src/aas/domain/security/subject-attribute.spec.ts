...
describe("subjectAttribute", () => {
  it("should evaluate isEqual", () => {
    ...
    const adminWithMemberRole = SubjectAttributes.create({ userRole: UserRole.ADMIN, memberRole: MemberRole.OWNER });
    expect(admin.isEqual(adminWithMemberRole)).toBeTruthy();
    const user = SubjectAttributes.create({ userRole: UserRole.USER, memberRole: MemberRole.MEMBER });
    const otherUser = SubjectAttributes.create({ userRole: UserRole.USER, memberRole: MemberRole.MEMBER });
    const owner = SubjectAttributes.create({ userRole: UserRole.USER, memberRole: MemberRole.OWNER });
  ...
  it("should get subjects with subordinated roles ", () => {
    ...
    expect(adminWithMemberRole.getSubjectsWithSubordinatedRoles()).toEqual([owner, member, user, anonymous]);
