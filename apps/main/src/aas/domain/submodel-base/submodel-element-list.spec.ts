...
describe("submodelElementList", () => {
  ...
  it("should add submodel element", () => {
    ...
    expect(() =>
      submodelElementList.addSubmodelElement(
        SubmodelElementCollection.create({ idShort: "sec1" }),
        { ability },
      ),
    ).toThrow(
      new Error(
  ...
  it("should delete submodel element", () => {
    ...
    security.addPolicy(member, IdShortPath.create({ path: "list" }), [
      Permission.create({ permission: Permissions.Create, kindOfPermission: PermissionKind.Allow }),
      Permission.create({ permission: Permissions.Read, kindOfPermission: PermissionKind.Allow }),
    ...
    expect(() =>
      submodelElementList.deleteSubmodelElement("unknown", { ability, onDelete }),
    ).toThrow(ValueError);
  ...
  it("should get values readable by specified subject", () => {
    ...
    security.addPolicy(member, IdShortPath.create({ path: "list.row.prop1" }), [
      Permission.create({ permission: Permissions.Read, kindOfPermission: PermissionKind.Allow }),
    ]);
    ...
    expect(submodelElementList.toPlain({ ability })).toEqual({
      ...submodelElementList.toPlain(),
      value: [{ ...row.toPlain(), value: [prop1.toPlain()] }],
    });
