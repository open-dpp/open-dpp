...
describe("submodelElementCollection", () => {
  ...
  it("should add submodel element", () => {
    ...
    expect(() => submodelElementCollection.addSubmodelElement(submodelElement, { ability })).toThrow(
  ...
  it("should get values readable by specified subject", () => {
    ...
    security.addPolicy(member, IdShortPath.create({ path: "subSection1" }), [Permission.create({ permission: Permissions.Read, kindOfPermission: PermissionKind.Allow }), Permission.create({ permission: Permissions.Create, kindOfPermission: PermissionKind.Allow })]);
