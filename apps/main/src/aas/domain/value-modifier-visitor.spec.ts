...
describe("value modifier visitor", () => {
  ...
  it("should modify value of property", () => {
    ...
    expect(() => submodel.modifyValueOfSubmodelElement(modifications, IdShortPath.create({ path: "prop1" }), { ability: abilityAnonymous })).toThrow(
  ...
  it("should modify value of file", () => {
    ...
    const anonymous = SubjectAttributes.create({ userRole: UserRole.ANONYMOUS });
    const abilityAnonymous = security.defineAbilityForSubject(anonymous);
    expect(() => submodel.modifyValueOfSubmodelElement(modifications, IdShortPath.create({ path: "file" }), { ability: abilityAnonymous })).toThrow(
      new ForbiddenError(`${prefixPermissionError} ${submodel.idShort}.file.`),
    );
  });
  ...
  it("should modify value of reference element", () => {
    ...
    const anonymous = SubjectAttributes.create({ userRole: UserRole.ANONYMOUS });
    const abilityAnonymous = security.defineAbilityForSubject(anonymous);
    expect(() => submodel.modifyValueOfSubmodelElement(modifications, path, { ability: abilityAnonymous })).toThrow(
      new ForbiddenError(`${prefixPermissionError} ${submodel.idShort}.ref.`),
    );
  });
  ...
  it("should modify value of submodel element list", () => {
    ...
    submodel.modifyValueOfSubmodelElement(modifications, IdShortPath.create({ path: "list" }), { ability });
    ...
    expect(multiLanguageProperty.displayName).toEqual(existingDisplayNames.map(LanguageText.fromPlain));
    const anonymous = SubjectAttributes.create({ userRole: UserRole.ANONYMOUS });
    const abilityAnonymous = security.defineAbilityForSubject(anonymous);
    expect(() => submodel.modifyValueOfSubmodelElement(modifications, IdShortPath.create({ path: "list" }), { ability: abilityAnonymous })).toThrow(
