...
**Cards are a first-class pattern in this app.** The operator UI and the public viewer both rely on cards to delimit Submodels, section data, and frame key values. The project-level constraint explicitly authorizes card usage — the general "no generic rounded-shadow cards" guidance from the base skill is overridden here. What matters is not whether cards are used, but *how*:
